...
class Api extends CI_Controller {
    ...
    public function add_info() {
        ...
        $info = json_decode( htmlspecialchars_decode( $_POST['info'] ) ,true);
        $type = (int)$this->input->post('type',true);
        if($type==1){
            $base64info_tmp = $this->input->post('base64info',true);
        } elseif($type==2) {
            $base64info_tmp = json_decode( htmlspecialchars_decode( $_POST['base64info'] ), true);
        }
        ...
        if($type==1) {
            $base64info = _get_img_frm_base64( $base64info_tmp );
        ...
        if($type==2) {
            foreach ($base64info_tmp as $k => $v) {
                ...
                $base64info[$k]['info'] = _get_img_frm_base64($v);
            }
